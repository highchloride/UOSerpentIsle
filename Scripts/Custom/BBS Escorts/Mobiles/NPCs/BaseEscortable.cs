...
namespace Server.Mobiles
{
    public class BaseEscortable : BaseCreature
    {
        ...
        public virtual bool SayDestinationTo(Mobile m)
        {
            ...
            if (escorter == null)
            {
                this.Say("I am looking to go to {0}, will you take me?", (dest.Name == "Ocllo" && m.Map == Map.SerpentIsle) ? "Haven" : dest.Name);
                return true;
            }
            else if (escorter == m)
            {
                this.Say("Lead on! Payment will be made when we arrive in {0}.", (dest.Name == "Ocllo" && m.Map == Map.SerpentIsle) ? "Haven" : dest.Name);
                return true;
            }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            this.m_Message = reader.ReadItem() as BulletinMessage;
            #endregion
        }
        ...
        {
            if (Map.SerpentIsle.Regions.Count == 0 || this.Map == null || this.Map == Map.Internal || this.Location == Point3D.Zero)
        ...
        public EDI GetDestination()
        {
            ...
            if (Map.SerpentIsle.Regions.Count > 0)
    ...
    public class EscortDestinationInfo
    {
        ...
        {
            ICollection list = Map.SerpentIsle.Regions.Values; //UOSI
