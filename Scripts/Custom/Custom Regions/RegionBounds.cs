...
namespace Server.Commands
{
    public class RegionBounds
    {
        ...
        private class RegionBoundTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Mobile)
                {
                    ...
                    from.SendMessage(String.Format("That Mobile's region is of type {0}, with a priority of {1}.",
                        r.GetType().FullName, r.Priority));
        ...
        private static void ShowRectBounds(Rectangle3D r, Map m, Mobile from, bool control, bool active)
        {
            ...
            Point3D p1 = new Point3D(r.Start.X, r.Start.Y - 1, from.Z);
                //So we dont' need to create a new one each point
            Point3D p2 = new Point3D(r.Start.X, r.Start.Y + r.Height - 1, from.Z);
                //So we dont' need to create a new one each point

            Effects.SendLocationEffect(new Point3D(r.Start.X - 1, r.Start.Y - 1, from.Z), m, 251, 75, 1, hue, 3);
        ...
        private static void ShowRegionBounds(Region r, Mobile from, bool control, bool active)
        {
            if (r == null)
            {
                return;
