...
namespace Server.Mobiles
{
	public class XmlSpawner : Item, ISpawner
	{
		...
		*/
        ...
        //UOSI - Rain trigger
        [CommandProperty(AccessLevel.GameMaster)]
        public bool RequireRaining
        {
            get { return m_RequireRaining; }
            set { m_RequireRaining = value; }
        }
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		public void CheckTriggers(Mobile m, Skill s, bool hasproximity)
		{
			...
			{
                bool needs_rain_trigger = false; //UOSI
                ...
                //UOSI check for raining
                if(m_RequireRaining)
                {
                    needs_rain_trigger = true;
                }

                //UOSI if Require Raining check for successful
                if(needs_rain_trigger)
                {
                    bool isRaining = false;

                   List<Misc.Weather> weathers = Misc.Weather.GetWeatherList(this.Map);
                   foreach(Misc.Weather weather in weathers)
                    {
                        if (!weather.Active)
                            break;
                        if (weather.Bounds.Contains(this.Location) && weather.Active)
                            isRaining = true;
                    }
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
                case 32: //UOSI
                    {
                        m_RequireRaining = reader.ReadBool();
                        goto case 31;
                    }
				case 31:
					{
