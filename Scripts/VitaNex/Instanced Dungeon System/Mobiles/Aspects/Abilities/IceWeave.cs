...
{
	public class AspectAbilityIceWeave : WaveAspectAbility
	{
		...
		protected override BaseWaveEffect CreateEffect(BaseAspect aspect)
		{
			...
			return new AirWaveEffect(aspect.Location, aspect.Map, dir, Math.Max(5, aspect.RangePerception / 2));
		}
		...
		protected override void OnAdded(State state)
		{
			...
			state.Target.SendMessage(state.Aspect.SpeechHue, "[{0}]: Freeze!", state.Aspect.RawName);
